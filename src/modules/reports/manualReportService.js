...
module.exports={
  ...
  async assign(periodId,teamCode,body,actorId){const userId=body?.userId||null;teamCode=String(teamCode).toUpperCase();const result=await repository.assignSubmission({periodId,teamCode,userId,actorId});if(!result)throw new HttpError('Không tìm thấy phiếu đang mở để phân công.',404,'REPORT_WORKSPACE_NOT_FOUND');if(result.invalidUser)throw new HttpError('Người được chọn không hoạt động hoặc chưa có quyền nhập/duyệt báo cáo.',400,'REPORT_ASSIGNEE_INVALID');if(result.locked)throw new HttpError('Không thể đổi người thực hiện khi phiếu đang chờ duyệt hoặc đã duyệt.',409,'REPORT_ASSIGNMENT_LOCKED');return repository.getPeriod(periodId);},
