...
module.exports={
  ...
  async create({teamCode,name,unit,evaluationDirection,userId}){
    return db.transaction(async client=>{
      ...
      const result=await client.query(`INSERT INTO report_kpi_definitions(team_id,code,name,unit,evaluation_direction,aggregation_method,display_order,input_mode,is_active)
        VALUES($1,$2,$3,$4,$5,'non_aggregatable',$6,'manual',TRUE) RETURNING *`,[team.id,code,name,unit,evaluationDirection,order]);
      await audit(client,result.rows[0].id,'created',userId,null,result.rows[0]);return result.rows[0];
    });
  },
  async update(id,{name,unit,evaluationDirection,isActive,userId}){
    return db.transaction(async client=>{
      ...
      const result=await client.query(`UPDATE report_kpi_definitions SET name=$2,unit=$3,evaluation_direction=$4,is_active=COALESCE($5,is_active),updated_at=CURRENT_TIMESTAMP WHERE id=$1 RETURNING *`,[id,name,unit,evaluationDirection,isActive]);
