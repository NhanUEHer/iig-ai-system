...
const payload=body=>{const name=required(body?.name,'Tên KPI'),unit=required(body?.unit,'Đơn vị');const evaluationDirection=String(body?.evaluationDirection||'');if(!DIRECTIONS.has(evaluationDirection))throw new HttpError('Chiều đánh giá không hợp lệ.',400,'REPORT_KPI_DIRECTION_INVALID');return{name,unit,evaluationDirection};};
module.exports={
