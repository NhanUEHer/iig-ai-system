...
function buildTransactionWorkbook(rows){
  const data=[HEADERS,...rows.map((row,index)=>[index+1,asDate(row.transaction_date),asDate(row.posting_date),transactionType(row),row.bank_code,row.account_name,row.account_number_masked,row.account_type,row.account_currency,asNumber(row.original_amount),row.original_currency||row.account_currency||'VND',asNumber(row.debit_amount),asNumber(row.credit_amount),asNumber(row.fee_amount),asNumber(row.debit_amount),row.expense_category||'Chi phí khác',row.expense_subcategory||'Chưa xác định',row.fee_type||'',row.cost_nature==='fee'?'Phí':'Chi phí trực tiếp',row.matched_keyword||'',row.classification_confidence||'',row.description,row.original_filename,row.source_page||'',row.source_row||'',row.committed_at?new Date(row.committed_at):null])];
  const sheet=XLSX.utils.aoa_to_sheet(data,{cellDates:true});
  sheet['!cols']=[7,16,22,17,13,24,21,17,16,23,23,17,17,18,18,24,30,28,19,22,16,65,36,12,16,22].map(wch=>({wch}));
  sheet['!autofilter']={ref:`A1:Z${data.length}`};
  for(let row=2;row<=data.length;row++){for(const column of ['B','C'])if(sheet[`${column}${row}`])sheet[`${column}${row}`].z='dd/mm/yyyy';if(sheet[`Z${row}`])sheet[`Z${row}`].z='dd/mm/yyyy hh:mm';for(const column of ['J','L','M','N','O'])if(sheet[`${column}${row}`])sheet[`${column}${row}`].z='#,##0.00';}
