...
const API_TYPES = new Set(['Grading', 'STT']);
...
function validate(body, { existingKey = false } = {}) {
  ...
  if (!name || !API_TYPES.has(apiType) || !targetQuestions.length) throw new HttpError('Vui lòng nhập tên, loại Agent và ít nhất một nhóm câu hỏi.', 400, 'VALIDATION_ERROR');
  ...
  return { name, description: String(body.description || '').trim() || null, apiEndpoint: normalizeEndpoint(body.api_endpoint), apiKey, apiType, sttTarget, targetQuestions };
}
...
router.get('/', requirePermission('agents.view'), asyncHandler(async (req, res) => {
  ...
  const apiType = ['Grading', 'STT'].includes(req.query.type) ? req.query.type : null;
  const params = [search];
...
router.post('/', requirePermission('agents.manage'), asyncHandler(async (req, res) => {
  ...
  if (duplicate.rows.length) throw new HttpError('Đã có Agent cùng loại/đích phụ trách một trong các nhóm câu hỏi này.', 409, 'AGENT_CONFLICT');
