...
import { 
  ...
  Cpu, Sliders, Settings, Eye, EyeOff, Edit2, Download
} from 'lucide-react';

const API_BASE = 'http://localhost:5005/api/submissions';
const AUTH_BASE = 'http://localhost:5005/api/auth';
...
const AudioPlayer = ({ fileId }) => {
  ...
  if (loading) {
    return <div style={{ fontSize: '12px', color: '#64748b', fontStyle: 'italic' }}>⏳ Đang lấy link audio...</div>;
  }

  if (error) {
    return <div style={{ fontSize: '12px', color: '#ef4444', fontStyle: 'italic' }}>❌ Lỗi tải: {error}</div>;
  }

  if (!url) {
    return <div style={{ fontSize: '12px', color: '#ef4444', fontStyle: 'italic' }}>⚠️ Không tìm thấy URL file âm thanh.</div>;
  }

  return (
    <div style={{ marginTop: '5px' }}>
      <audio src={url} controls style={{ width: '100%', maxWidth: '380px', height: '40px' }} />
    </div>
  );
};
...
const CardSection = ({ icon, label, children }) => (
  <div style={{ borderRadius: '12px', border: '1px solid rgba(255, 255, 255, 0.06)', overflow: 'hidden', marginBottom: '12px', background: 'rgba(255, 255, 255, 0.02)', backdropFilter: 'blur(12px)' }}>
...
// ── ModernWaveformPlayer: Custom Audio Player style from concept ──
const ModernWaveformPlayer = ({ fileId }) => {
  const [url, setUrl] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
    ...
    const fetchUrl = async () => {
      try {
        ...
        if (res.data && res.data.success && res.data.url) {
          ...
          setError(res.data?.message || 'Không có URL');
        }
      } catch (e) {
        setError(e.message || 'Lỗi mạng');
      } finally {
  ...
  if (loading) return <div style={{ fontSize: '12px', color: 'var(--on-surface-var)', fontStyle: 'italic' }}>⏳ Đang tải tệp âm thanh...</div>;
  if (error || !url) return <div style={{ fontSize: '12px', color: 'var(--error)', fontStyle: 'italic' }}>⚠️ Lỗi tải file.</div>;

  return (
    <div className="glass-container" style={{
      display: 'flex', alignItems: 'center', gap: '16px', padding: '16px', borderRadius: '16px',
      background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(168, 200, 255, 0.1)', width: '100%', maxWidth: '520px'
    }}>
      <audio ref={audioRef} src={url} onTimeUpdate={onTimeUpdate} onLoadedMetadata={onLoadedMetadata} onEnded={() => setIsPlaying(false)} />
      
      {/* Play Icon */}
      <button onClick={handlePlayPause} style={{
        width: '44px', height: '44px', borderRadius: '50%', background: 'var(--primary-container)', color: '#fff',
        display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: 'none', transition: 'transform 0.2s'
      ...
      {/* Progress & Waveform mockup */}
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: '2px', height: '18px', marginBottom: '8px', opacity: 0.6 }}>
          {[12, 16, 8, 14, 20, 10, 15, 9, 7, 13, 18, 11, 14, 6, 12, 19, 8, 15, 10, 12, 16, 7, 13, 18].map((h, i) => (
            <div key={i} style={{
              flex: 1, height: `${h}px`, borderRadius: '1px',
              background: isPlaying && progress > (i / 24) * 100 ? 'var(--primary)' : 'rgba(255, 255, 255, 0.15)'
            }} />
          ))}
        </div>
        <div style={{ position: 'relative', height: '4px', background: 'rgba(255,255,255,0.1)', borderRadius: '9999px' }}>
          <div style={{ position: 'absolute', left: 0, top: 0, height: '100%', background: 'var(--primary)', width: `${progress}%`, borderRadius: '9999px' }}>
            <div style={{ position: 'absolute', right: 0, top: '50%', transform: 'translate(-50%, -50%)', width: '8px', height: '8px', borderRadius: '50%', background: '#fff', border: '2px solid var(--primary)' }} />
          </div>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '6px', fontSize: '11px', color: 'var(--on-surface-var)' }}>
...
const AnswerCard = ({ ans, answerStatusBadge, onGradeComplete }) => {
  ...
  const accentColor  = isWriting ? '#0055a5' : '#00677e';
  const accentBg     = isWriting ? 'rgba(0, 85, 165, 0.08)'  : 'rgba(0, 103, 126, 0.08)';
  const accentBorder = isWriting ? 'rgba(0, 85, 165, 0.20)'  : 'rgba(0, 103, 126, 0.20)';
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        {/* Q1-2: Read Aloud */}
        {qType === 'sp_read_aloud' && (
          <CardSection icon="description" label="Đề bài — Đoạn văn cần đọc">
            <p style={{ margin: 0, fontSize: '15.5px', color: 'var(--on-surface)', lineHeight: 2.0, fontWeight: '500' }}>
              {ans.prompt_text || <em style={{ color: 'var(--on-surface-var)' }}>Không có nội dung</em>}
            </p>
          </CardSection>
        )}

        {/* Q3-4: Describe a Picture */}
        {qType === 'sp_describe_pic' && (
          <CardSection icon="image" label="Đề bài — Hình ảnh mô tả">
            {ans.image_url
              ? <img src={ans.image_url} alt={`Ảnh câu ${ans.question_no}`}
                  style={{ width: '100%', maxHeight: '500px', objectFit: 'contain', borderRadius: '8px', display: 'block', margin: '0 auto', border: '1px solid rgba(255,255,255,0.08)' }} />
              : <em style={{ color: 'var(--on-surface-var)', fontSize: '14px' }}>Không có hình ảnh</em>}
          </CardSection>
        )}

        {/* Q5-7: Respond to Questions */}
        {qType === 'sp_respond_q' && (<>
          <CardSection icon="language" label="Tình huống">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {ans.prompt_text?.trim() && (
                <p style={{ margin: 0, fontSize: '14.5px', color: 'var(--on-surface)', lineHeight: 1.8 }}>
                  {ans.prompt_text}
                </p>
              )}
              <AudioRow label="Audio tình huống" fileId={ans.context_audio_file_id} />
            </div>
          </CardSection>

          <CardSection icon="help" label="Câu hỏi">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {ans.question_name?.trim() && (
                <p style={{ margin: 0, fontSize: '15.5px', fontWeight: '600', color: 'var(--on-surface)', lineHeight: 1.7 }}>
                  {ans.question_name}
                </p>
              )}
              <AudioRow label="Audio câu hỏi" fileId={ans.question_audio_file_id} />
            </div>
          </CardSection>
        </>)}

        {/* Q8-10: Respond with Info */}
        {qType === 'sp_respond_info' && (<>
          {(ans.image_url || ans.context_text?.trim() || ans.context_audio_file_id) && (
            <CardSection icon="assignment" label="Tình huống & Thông tin">
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {ans.image_url && (
                  <img src={ans.image_url} alt="Bảng thông tin"
                    style={{ maxWidth: '100%', maxHeight: '340px', objectFit: 'contain', borderRadius: '8px', display: 'block', border: '1px solid rgba(255,255,255,0.08)' }} />
                )}
                {ans.context_text?.trim() && (
                  <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '8px', padding: '12px 16px', fontSize: '14px', color: 'var(--on-surface-var)', lineHeight: 1.8 }}>
                    <span style={{ fontWeight: '700', display: 'block', marginBottom: '4px', color: 'var(--on-surface)' }}>📝 Transcript:</span>
                    {ans.context_text}
                  </div>
                )}
                <AudioRow label="Audio tình huống" fileId={ans.context_audio_file_id} />
        ...
        {qType === 'sp_opinion' && (
          <CardSection icon="chat" label="Câu hỏi phát biểu ý kiến">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {ans.prompt_text?.trim() && (
                <p style={{ margin: 0, fontSize: '15.5px', color: 'var(--on-surface)', lineHeight: 1.9, fontWeight: '500' }}>
                  {ans.prompt_text}
                </p>
              )}
              <AudioRow label="Audio câu hỏi" fileId={ans.context_audio_file_id} />
            </div>
          </CardSection>
        )}

        {/* ── Student Response Card Section (Audio player + AI Transcribed text / Text response) ── */}
        <CardSection icon="account_circle" label="Bài làm của học viên">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {isWriting ? (
              <div style={{ 
                ...
                {ans.student_writing || <em style={{ color: 'var(--on-surface-var)', opacity: 0.5 }}>Không có nội dung câu trả lời viết</em>}
              </div>
            ) : (
              <>
                {ans.student_audio_file_id ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <span style={{ fontSize: '11px', fontWeight: '700', color: 'var(--on-surface-var)', display: 'flex', alignItems: 'center', gap: '4px', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                      <span className="material-symbols-outlined" style={{ fontSize: '14px' }}>mic</span> File ghi âm học viên
                    </span>
                    <ModernWaveformPlayer fileId={ans.student_audio_file_id} />
                  </div>
                ) : (
                  <em style={{ color: 'var(--on-surface-var)', opacity: 0.5, fontSize: '13px' }}>Chưa nộp file ghi âm</em>
                )}

                {/* Live transcription text block if available */}
                {ans.transcribe && (
                  <div style={{ 
                    marginTop: '8px',
                    ...
                    <span style={{ 
                      ...
                      background: '#131b2e', padding: '2px 8px', borderRadius: '4px',
                      fontSize: '10px', fontWeight: 'bold', color: '#38bdf8', border: '1px solid rgba(56, 189, 248, 0.25)' 
                    }}>
                      📝 AI TRANSCRIBED TEXT (STT)
                    </span>
        ...
        {/* Giao diện nút kích hoạt AI Agents động */}
        <div style={{
          marginTop: '12px', display: 'flex', alignItems: 'center', gap: '10px', padding: '12px',
          border: '1px dashed rgba(168, 200, 255, 0.15)', borderRadius: '12px', background: 'rgba(255, 255, 255, 0.01)', flexWrap: 'wrap'
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--primary)', marginRight: 'auto' }}>
            <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>smart_toy</span>
            <span style={{ fontSize: '12px', fontWeight: 'bold' }}>AI Copilot Assistants</span>
          </div>

          <div style={{ display: 'flex', gap: '10px' }}>
            {/* AI Transcribe (Chỉ áp dụng cho Speaking) */}
            {ans.section === 'Speaking' && ans.student_audio_file_id && (
              <button 
                onClick={() => {
                  if (onGradeComplete) {
                    onGradeComplete(ans.id, 'STT');
                  }
                }}
                title="Dịch giọng nói sang văn bản (STT)"
                style={{
                  display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px', borderRadius: '8px',
                  background: 'rgba(0, 103, 126, 0.15)', border: '1px solid rgba(0, 103, 126, 0.25)',
                  color: '#38bdf8', cursor: 'pointer', fontSize: '12.5px', fontWeight: 'bold'
                }}
            ...
            {/* AI Grading (Áp dụng cho cả Speaking và Writing) */}
            {((ans.section === 'Speaking' && ans.student_audio_file_id) || (ans.section === 'Writing' && ans.student_writing)) && (
              <button 
                onClick={() => {
                  if (onGradeComplete) {
                    onGradeComplete(ans.id, 'Grading');
                  }
                }}
                title="Gửi sang AI Agent chấm điểm chi tiết bài thi"
                style={{
                  display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 14px', borderRadius: '8px',
                  background: 'rgba(0, 85, 165, 0.15)', border: '1px solid rgba(0, 85, 165, 0.25)',
                  color: '#60a5fa', cursor: 'pointer', fontSize: '12.5px', fontWeight: 'bold'
                }}
                className="hover:opacity-80 active:scale-95"
              >
...
const SubmissionDetailView = ({ showMsg, getStatusBadge, addLiveLog }) => {
  ...
  const handleGradeAI = async (answerId, apiTypeVal = 'STT') => {
    ...
    try {
      ...
      const agentsRes = await axios.get('http://localhost:5005/api/agents');
      ...
      const matchAgent = agentsRes.data.data.find(agent => 
        agent.api_type === apiTypeVal && 
        (agent.target_questions || []).includes(qTypeKey)
      );

      if (!matchAgent) {
        showMsg('error', `Không tìm thấy Agent loại ${apiTypeVal === 'STT' ? 'Transcribe' : 'Grading'} nào được gán cho câu hỏi nhóm: ${qTypeKey.replace('sp_', 'Speaking ').replace('w_', 'Writing ')}`);
        return;
      }

      showMsg('info', `Kích hoạt Agent: ${matchAgent.name}...`);
      if (addLiveLog) {
        addLiveLog(`🤖 Kích hoạt Dify AI Agent [${matchAgent.name}] cho câu: ${labelQ}`);
      }

      const targetEndpoint = apiTypeVal === 'STT' ? 'transcribe-ai' : 'grade-ai';
      ...
      if (res.data.success) {
        showMsg('success', `${apiTypeVal === 'STT' ? 'STT' : 'Chấm điểm'} hoàn tất!`);
        if (addLiveLog) {
          addLiveLog(`🟢 AI Agent [${matchAgent.name}] trả kết quả chấm câu: ${labelQ} thành công.`);
        }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', background: 'var(--bg)', padding: '24px', position: 'relative' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingRight: '4px', position: 'relative', zIndex: 1 }}>
        ...
        {/* === CANDIDATE INFO CARD === */}
        <section className="glass-card" style={{ padding: '24px', marginBottom: '32px', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '24px', position: 'relative', overflow: 'hidden' }}>
          {/* AI Accent Glow */}
          ...
          <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
            <div style={{
              ...
              display: 'flex', alignItems: 'center', justifySelf: 'center', justifyContent: 'center', fontWeight: '700', fontSize: '24px', boxShadow: '0 8px 32px rgba(0,85,165,0.2)'
            }}>
              {(data.student_name || 'N').charAt(0).toUpperCase()}
            </div>
            <div>
              <h3 style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: 'var(--on-surface)' }}>{data.student_name}</h3>
              <p style={{ margin: '4px 0 12px 0', fontSize: '14px', color: 'var(--on-surface-var)' }}>{data.test_name}</p>
              <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap', fontSize: '12px', color: 'var(--on-surface-var)' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <span className="material-symbols-outlined" style={{ fontSize: '16px' }}>mail</span> {data.student_email || 'Chưa có email'}
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <span className="material-symbols-outlined" style={{ fontSize: '16px' }}>call</span> {data.student_phone || 'Chưa có SĐT'}
                </span>
                <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  <span className="material-symbols-outlined" style={{ fontSize: '16px' }}>event_available</span> Nộp: {new Date(data.submitted_date).toLocaleString('vi-VN')}
                </span>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
            <div className="glass-card" style={{ padding: '12px 24px', borderRadius: '12px', textAlign: 'center', minWidth: '100px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(168,200,255,0.1)' }}>
              <p style={{ margin: 0, fontSize: '24px', fontWeight: '700', color: 'var(--primary)' }}>{data.answers?.length || 0}</p>
              <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: 'var(--on-surface-var)' }}>Tổng câu</p>
            </div>
            <div className="glass-card" style={{ padding: '12px 24px', borderRadius: '12px', textAlign: 'center', minWidth: '100px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(168,200,255,0.1)' }}>
              <p style={{ margin: 0, fontSize: '24px', fontWeight: '700', color: 'var(--on-surface)' }}>{writingAnswers.length}</p>
              <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: 'var(--on-surface-var)' }}>Writing</p>
            </div>
            <div className="glass-card" style={{ padding: '12px 24px', borderRadius: '12px', textAlign: 'center', minWidth: '100px', background: 'rgba(0, 85, 165, 0.05)', border: '1px solid rgba(0, 85, 165, 0.2)' }}>
              <p style={{ margin: 0, fontSize: '24px', fontWeight: '700', color: 'var(--primary)' }}>{speakingAnswers.length}</p>
              <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: 'var(--primary)' }}>Speaking</p>
            </div>
            
            <div style={{ height: '48px', width: '1px', background: 'rgba(255,255,255,0.1)', margin: '0 8px' }} />

            <div style={{ background: 'rgba(255,255,255,0.02)', padding: '12px 20px', borderRadius: '12px', textAlign: 'center', border: '1px solid rgba(255,255,255,0.06)', minWidth: '130px' }}>
              <p style={{ margin: 0, fontSize: '11px', color: 'var(--on-surface-var)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Keycode</p>
              <p style={{ margin: '4px 0 0 0', fontSize: '22px', fontWeight: '800', color: 'var(--on-surface)', letterSpacing: '0.05em' }}>{data.keycode}</p>
            </div>
          </div>
        </section>
          ...
          <div style={{ marginBottom: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
              ...
              <h3 style={{ margin: 0, fontSize: '15px', fontWeight: '700', color: '#15803d' }}>
                Speaking — {speakingAnswers.length} câu
              </h3>
            </div>
            {speakingAnswers.map((ans, i) => (
              <AnswerCard 
                key={ans.id} 
                ans={ans} 
                index={i} 
        ...
        {writingAnswers.length > 0 && (
          <div style={{ marginBottom: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
              <div style={{ width: '4px', height: '20px', background: '#2563eb', borderRadius: '2px' }}/>
              <h3 style={{ margin: 0, fontSize: '15px', fontWeight: '700', color: '#1e40af' }}>
                Writing — {writingAnswers.length} câu
              </h3>
            </div>
            {writingAnswers.map((ans, i) => (
              <AnswerCard 
                key={ans.id} 
                ans={ans} 
                index={i} 
...
function App() {
  ...
  const addLiveLog = (text) => {
    ...
    setLiveLogs(prev => {
      ...
      // Keep only last 15 log entries to avoid overflow
  ...
  // Pull single submission by Keycode (Elearning IIG -> local DB)
  const handlePullSubmission = async (e) => {
    e.preventDefault();
    if (!pullKeycode.trim()) return;
    setPullStatus('loading');
    addLiveLog(`📥 Bắt đầu kéo bài thi từ Elearning: ${pullKeycode.toUpperCase()}`);
    try {
      const res = await axios.post(`${API_BASE}/sync`, {
        keycode: pullKeycode.trim().toUpperCase()
      });
      if (res.data.success) {
        addLiveLog(`✅ Kéo thành công bài thi: ${pullKeycode.toUpperCase()}`);
        setPullResultId(res.data.data?.id || res.data.id || '');
        setPullStatus('success');
        fetchSubmissions();
      } else {
        addLiveLog(`❌ Kéo bài thi ${pullKeycode.toUpperCase()} thất bại.`);
        setPullStatus('error');
      }
    } catch (error) {
      addLiveLog(`❌ Lỗi kéo bài thi ${pullKeycode.toUpperCase()}: ${error.message}`);
      setPullStatus('error');
      showMsg('error', 'Kéo bài thi thất bại: ' + (error.response?.data?.error || error.message));
    }
  };
    ...
    <div style={{ display: 'flex', height: '100vh', width: '100%', background: 'var(--bg)', overflow: 'hidden', position: 'relative' }}>
        ...
        <Routes>
          <Route path="/submissions" element={
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', padding: '24px 24px 0' }}>
                {/* Total */}
                <div className="stat-card">
                  ...
                  <div className="mini-bar" style={{ display: 'flex', alignItems: 'flex-end', gap: '4px', height: '24px', marginTop: '8px' }}>
                    {(() => {
                ...
                <div className="stat-card">
                  ...
                  {submissions.length > 0 && (() => { const pct = Math.round(submissions.filter(s=>s.status===3).length/submissions.length*100); return (
                    <><div className="progress-bar" style={{ marginTop: '8px' }}><div className="progress-fill" style={{ width: `${pct}%` }} /></div><p style={{ margin: '4px 0 0', fontSize: '10px', color: 'var(--on-surface-var)' }}>{pct}% hoàn thành</p></>
                  );})()} 
                </div>
                {/* Pending */}
                <div className="stat-card">
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '8px' }}>
                    <RefreshCw size={13} style={{ color: 'var(--primary)' }} className={submissions.filter(s=>s.status===2).length>0?'spinner pulse-dot':''}/>
                    <span style={{ fontSize: '11px', color: 'var(--on-surface-var)' }}>Đang trong hàng đợi...</span>
                  </div>
                </div>
              </div>
          ...
          <Route path="/logs" element={
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', padding: '24px' }}>
              ...
              <div className="glass-card" style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#0b121f', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '16px', overflow: 'hidden' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '6px', fontFamily: 'Courier New, monospace', fontSize: '12.5px', color: '#cbd5e1', lineHeight: 1.6 }}>
      ...
      {showSyncModal && (
        <div style={{
          ...
          <div className="glass-card" style={{ width: '100%', maxWidth: '440px', padding: '32px', boxSizing: 'border-box' }}>
              ...
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      <label style={{ fontSize: '11px', fontWeight: '600', color: '#475569' }}>TỪ NGÀY NỘP</label>
                      <input 
                        type="date" 
                        value={syncFromDate} 
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      <label style={{ fontSize: '11px', fontWeight: '600', color: '#475569' }}>ĐẾN NGÀY NỘP</label>
                      <input 
                        type="date" 
                        value={syncToDate} 
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <label style={{ fontSize: '11px', fontWeight: '600', color: '#475569' }}>TỪ KHÓA TÌM KIẾM</label>
                    <input 
                      type="text" 
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <label style={{ fontSize: '11px', fontWeight: '600', color: '#475569' }}>SỐ LƯỢNG TẢI (MAX PAGE SIZE)</label>
                    <input 
                      type="number" 
                      min="10" 
                      max="2000"
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '12px' }}>
                    ...
                    <button 
                      ...
                      className="btn-primary"
                      style={{ padding: '8px 16px', fontSize: '13px', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', border: 'none' }}
                    >
                      Xác nhận đồng bộ
                    </button>
                  </div>
            ...
            {syncStatus === 'loading' && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 0', gap: '16px' }}>
                <RefreshCw className="spinner" size={40} style={{ color: '#059669' }}/>
                <div style={{ fontSize: '15px', fontWeight: '600', color: '#0f172a', textAlign: 'center' }}>Đang đồng bộ dữ liệu...</div>
                <div style={{ fontSize: '12px', color: '#64748b', textAlign: 'center' }}>Hệ thống đang truy xuất bài thi và ánh xạ keycode từ IIG Elearning. Quá trình này có thể mất vài chục giây.</div>
              </div>
            )}
            ...
            {syncStatus === 'success' && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '12px 0', gap: '16px' }}>
                ...
                <div style={{ fontSize: '16px', fontWeight: '700', color: '#0f172a' }}>Đồng bộ hoàn tất!</div>
                <div style={{ fontSize: '14px', color: '#374151', textAlign: 'center', background: '#f0fdf4', padding: '10px 18px', borderRadius: '8px', border: '1px solid #d1fae5' }}>
                  Đã ánh xạ và đồng bộ thành công <strong style={{ color: '#059669', fontSize: '16px' }}>{syncCount}</strong> keycode từ Elearning.
                </div>
                <button 
                  onClick={() => setShowSyncModal(false)}
                  className="btn-primary"
                  style={{ padding: '8px 24px', fontSize: '13px', background: '#059669', border: 'none', marginTop: '8px' }}
                >
                  Hoàn thành
                </button>
              </div>
            ...
            {syncStatus === 'error' && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '12px 0', gap: '16px' }}>
                ...
                <div style={{ fontSize: '16px', fontWeight: '700', color: '#0f172a' }}>Không thể đồng bộ</div>
                <div style={{ fontSize: '13px', color: '#64748b', textAlign: 'center' }}>Vui lòng kiểm tra lại cấu hình bộ lọc hoặc phiên đăng nhập Elearning của bạn.</div>
      ...
      {showPullModal && (
        <div style={{
          ...
          <div className="glass-card" style={{ width: '100%', maxWidth: '420px', padding: '32px', boxSizing: 'border-box', background: 'rgba(23, 31, 51, 0.95)', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            {pullStatus === 'idle' && (
              <>
                <h3 style={{ margin: '0 0 20px 0', fontSize: '17px', fontWeight: '700', color: 'var(--on-surface)', display: 'flex', alignItems: 'center', gap: '8px', fontFamily: 'Lexend, sans-serif' }}>
                  ...
                  Kéo dữ liệu bài thi từ Elearning
                </h3>
                
                <form onSubmit={handlePullSubmission} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <label style={{ fontSize: '11px', fontWeight: 'bold', color: 'var(--on-surface-var)', letterSpacing: '0.05em' }}>MÃ KEYCODE BÀI THI</label>
                    <input 
                      type="text" 
                      placeholder="Ví dụ: MNTT4A, Z9FH65..." 
                      value={pullKeycode} 
                      onChange={(e) => setPullKeycode(e.target.value)} 
                      className="glass-input"
                      style={{ fontSize: '14px', padding: '10px 14px', textTransform: 'uppercase', fontWeight: 'bold', letterSpacing: '1px', background: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.08)' }}
                      required
                      autoFocus
                    />
                  </div>
            ...
            {pullStatus === 'loading' && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px 0', gap: '16px' }}>
                ...
                <div style={{ fontSize: '15px', fontWeight: '600', color: 'var(--on-surface)', textAlign: 'center' }}>Đang kéo dữ liệu bài thi...</div>
                <div style={{ fontSize: '12.5px', color: 'var(--on-surface-var)', textAlign: 'center', lineHeight: 1.7 }}>Hệ thống đang truy xuất file câu hỏi, câu trả lời, ghi âm bài thi của keycode <strong style={{ color: 'var(--primary)' }}>{pullKeycode.toUpperCase()}</strong> từ Elearning IIG. Vui lòng chờ trong giây lát.</div>
              </div>
            )}
            ...
            {pullStatus === 'success' && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '12px 0', gap: '16px' }}>
                ...
                <div style={{ fontSize: '16px', fontWeight: '700', color: 'var(--on-surface)' }}>Kéo bài thi thành công!</div>
                <div style={{ fontSize: '12.5px', color: 'var(--on-surface-var)', textAlign: 'center', lineHeight: 1.7 }}>Đã đồng bộ trọn vẹn dữ liệu đề bài, câu trả lời và kết quả chấm của bài thi lên hệ thống local.</div>
                  ...
                  {pullResultId && (
                    <button 
                      onClick={() => {
                        ...
                        // Kích hoạt xem chi tiết bài thi
            ...
            {pullStatus === 'error' && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '12px 0', gap: '16px' }}>
                ...
                <div style={{ fontSize: '16px', fontWeight: '700', color: 'var(--on-surface)' }}>Kéo bài thi thất bại</div>
                <div style={{ fontSize: '12.5px', color: 'var(--on-surface-var)', textAlign: 'center', lineHeight: 1.7 }}>Không tìm thấy bài thi hoặc keycode của bạn chưa có thông tin khớp trên Elearning API.</div>
...
const AgentManagementView = ({ showMsg }) => {
  ...
  const fetchAgents = async () => {
    setLoading(true);
    try {
      const res = await axios.get('http://localhost:5005/api/agents');
      if (res.data.success) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      let res;
      if (isEditing) {
        res = await axios.put(`http://localhost:5005/api/agents/${editingId}`, payload);
      } else {
        res = await axios.post('http://localhost:5005/api/agents', payload);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(`http://localhost:5005/api/agents/${id}`);
      if (res.data.success) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', padding: '24px' }}>
      ...
      {/* Agents Table List */}
      <div className="glass-card" style={{ flex: 1, overflowY: 'auto', borderRadius: '16px' }}>
        {loading ? (
          <div style={{ padding: '64px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
          ...
          <div style={{ padding: '80px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', opacity: 0.6 }}>
            <span className="material-symbols-outlined" style={{ fontSize: '48px', color: 'var(--on-surface-var)' }}>smart_toy</span>
            <span style={{ fontSize: '14px', color: 'var(--on-surface-var)' }}>Chưa có AI Agent nào được cấu hình.</span>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
                ...
                <tr key={agent.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <td style={{ padding: '16px 20px' }}>
                    <span style={{
                      ...
                      {agent.api_type === 'STT' ? 'Transcribe (STT)' : 'Grading (Chấm điểm)'}
                    </span>
                  </td>
