...
import { ArrowLeft, ArrowRight, Bot, Check, CheckCircle2, Edit3, Eye, EyeOff, KeyRound, Link2, Mic2, Plus, RefreshCw, Search, Sparkles, Trash2, X } from 'lucide-react';
...
export default function AIConfigPage({ showMsg }) {
  ...
  const setType = apiType => setForm(value => ({ ...value, api_type: apiType, stt_target: 'student_answer', target_questions: apiType === 'STT' ? value.target_questions.filter(key => key.startsWith('sp_')) : value.target_questions }));
  ...
  const save = async event => {
    ...
    if (!form.target_questions.length) return showMsg?.('Chọn ít nhất một nhóm câu hỏi.', 'error');
    setSaving(true);
    try {
  ...
  return <div className="agent-page">
    ...
    <section className="agent-metrics compact"><article><span className="blue"><Bot /></span><div><small>Tổng Agent</small><strong>{agents.length}</strong></div></article><article><span className="purple"><Sparkles /></span><div><small>Grading</small><strong>{gradingCount}</strong></div></article><article><span className="green"><Mic2 /></span><div><small>STT</small><strong>{sttCount}</strong></div></article></section>
    <section className="agent-list-card"><div className="agent-toolbar"><div className="agent-tabs"><button className={filter === 'all' ? 'active' : ''} onClick={() => { setFilter('all'); setPage(1); }}>Tất cả</button><button className={filter === 'Grading' ? 'active' : ''} onClick={() => { setFilter('Grading'); setPage(1); }}>Grading</button><button className={filter === 'STT' ? 'active' : ''} onClick={() => { setFilter('STT'); setPage(1); }}>STT</button></div><label><Search /><input value={search} onChange={event => { setSearch(event.target.value); setPage(1); }} placeholder="Tìm tên, mô tả hoặc endpoint…" /></label></div>
      {loading ? <div className="agent-empty"><RefreshCw className="spin" /><strong>Đang tải Agents…</strong></div> : !filtered.length ? <div className="agent-empty"><Bot /><strong>Không tìm thấy Agent</strong><span>Thêm cấu hình mới hoặc thay đổi bộ lọc.</span></div> : <div className="agent-table-wrap"><table className="agent-table"><thead><tr><th>Agent</th><th>Loại</th><th>Nhóm câu hỏi</th><th>Endpoint</th><th>Trạng thái</th><th>Cập nhật</th><th /></tr></thead><tbody>{filtered.map(agent => <tr key={agent.id}><td><div className="agent-identity"><span className={agent.api_type.toLowerCase()}>{agent.api_type === 'Grading' ? <Sparkles /> : <Mic2 />}</span><div><strong>{agent.name}</strong><small>{agent.description || 'Chưa có mô tả nhiệm vụ.'}</small></div></div></td><td><span className={`agent-type ${agent.api_type.toLowerCase()}`}>{agent.api_type === 'STT' ? `STT · ${TARGET_LABELS[agent.stt_target]}` : 'Grading'}</span></td><td><div className="agent-group-list">{agent.target_questions?.slice(0, 3).map(key => <span key={key}>{GROUPS.find(group => group.key === key)?.label || key}</span>)}{agent.target_questions?.length > 3 && <em>+{agent.target_questions.length - 3}</em>}</div></td><td><div className="agent-endpoint-cell"><Link2 /><span>{agent.api_endpoint}</span><KeyRound /></div></td><td><span className="agent-ready"><CheckCircle2 />Sẵn sàng</span></td><td><time>{new Date(agent.updated_at).toLocaleDateString('vi-VN')}</time></td><td><div className="agent-row-actions"><button onClick={() => openEdit(agent)} title="Chỉnh sửa"><Edit3 /></button><button className="danger" onClick={() => remove(agent)} title="Xóa"><Trash2 /></button></div></td></tr>)}</tbody></table></div>}
