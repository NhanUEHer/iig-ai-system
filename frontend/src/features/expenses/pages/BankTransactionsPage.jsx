...
import { ArrowLeft, ArrowRight, Download, ExternalLink, FileText, Landmark, RefreshCw, Search, X } from 'lucide-react';
...
export default function BankTransactionsPage({ showMsg }) {
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      const response=await api.get('/expenses/transactions/export',{params,responseType:'blob'});
      const disposition=response.headers['content-disposition']||'';
      const encodedName=disposition.match(/filename\*=UTF-8''([^;]+)/i)?.[1];
      const filename=encodedName?decodeURIComponent(encodedName):`giao-dich-ngan-hang_${new Date().toISOString().slice(0,10)}.xlsx`;
      const url=URL.createObjectURL(response.data);const anchor=document.createElement('a');anchor.href=url;anchor.download=filename;document.body.appendChild(anchor);anchor.click();anchor.remove();URL.revokeObjectURL(url);
      showMsgRef.current(`Đã xuất ${data.pagination.total||0} giao dịch ra Excel.`,'success');
    } catch(error){showMsgRef.current(error.response?.data?.error||'Không thể xuất dữ liệu giao dịch.','error');}
  ...
  return <div className="bank-transactions-page"><div className="bank-transactions-shell">
    <header className="bank-transactions-header"><div className="bank-transactions-title"><div><small>CHI PHÍ · GIAO DỊCH ĐÃ XÁC NHẬN</small><h1>Giao dịch ngân hàng</h1><p>Danh sách tổng hợp từ các sao kê đã kiểm tra và xác nhận import.</p></div></div><div className="bank-transactions-header-actions"><button className="export" onClick={exportExcel} disabled={exporting||loading}><Download/>{exporting?'Đang xuất…':'Xuất Excel'}</button><button onClick={() => load(data.pagination.page, filters)}><RefreshCw/>Làm mới</button></div></header>
