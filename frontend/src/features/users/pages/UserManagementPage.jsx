...
const emptyUser = { name: '', email: '', roleSlugs: [], primaryRoleSlug: '', isActive: true };
export default function UserManagementPage({ currentUser, showMsg }) {
  ...
  const openUser = user => {
    ...
    setUserForm(user ? { name: user.name, email: user.email, roleSlugs, primaryRoleSlug: user.roles?.find(role => role.isPrimary)?.slug || user.role || roleSlugs[0], isActive: user.is_active }
